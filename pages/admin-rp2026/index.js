    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', padding: '40px 36px', fontFamily: 'var(--font-body)', color: 'var(--text)' }}>
      <div style={{ maxWidth: 720, margin: '0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 28, flexWrap: 'wrap' }}>
          {SECTIONS.map(s => (
            <button key={s} className={`filter-tag${section === s ? ' active' : ''}`} onClick={() => setSection(s)}>{s}s</button>
          ))}
        </div>
            ...
            <>
              ...
              <div className="form-row single">
                <div className="form-group">
                  ...
                  <input className="form-input" style={inp} value={form.zoom_url || ''} onChange={e => setField('zoom_url', e.target.value)} placeholder="https://zoom.us/j/..." />
                </div>
              </div>
            </>
          )}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {items.map(item => (
                <div key={item.id} style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: 10, padding: '14px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
                  ...
                  <button onClick={() => startEdit(item)} style={{ background: 'rgba(0,245,228,0.08)', border: '1px solid rgba(0,245,228,0.2)', borderRadius: 6, padding: '6px 12px', color: 'var(--cyan)', fontFamily: 'var(--font-mono)', fontSize: 9, cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                    Edit
                  </button>
                  <button onClick={() => deleteItem(item.id)} style={{ background: 'rgba(255,45,120,0.08)', border: '1px solid rgba(255,45,120,0.2)', borderRadius: 6, padding: '6px 12px', color: 'var(--pink)', fontFamily: 'var(--font-mono)', fontSize: 9, cursor: 'pointer', letterSpacing: '0.1em', textTransform: 'uppercase', whiteSpace: 'nowrap' }}>
                    Delete
                  </button>
                </div>
              ))}
